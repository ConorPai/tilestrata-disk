...
var qsFunc = require("querystring");
...
FileSystemCache.prototype._fileFromDirectory = function(tile) {
	...
	//解析请求参数
	if (tile.qs != undefined && (tile.qs.indexOf('mzfilter') != -1 || tile.qs.indexOf('mzstyle') != -1)) {

		var ret = this.options.dir
		var paramMap = qsFunc.parse(tile.qs);
		if (paramMap.mzfilter != undefined && paramMap.mzfilter != '') {
			ret += '/' + paramMap.mzfilter;
		}

		if (paramMap.mzstyle != undefined && paramMap.mzstyle != '') {
			ret += '/' + paramMap.mzstyle.toUpperCase();
		}

		return ret  + '/' + tile.z + '/' + tile.x + '/' + tile.y + '/' + tile.filename;
	}
